...
class Primitive:
    ...
    def check_output_shape(self, x: list) -> bool:
        raise NotImplementedError
...
class ShrinkPrimitive(UnaryPrimitive):
    def __init__(self, output_shape):
        ...
        self.input_shapes = [(output_shape[0] // self.params["factor"], output_shape[1])]
...
class DiffPrimitive(UnaryPrimitive):
    def __init__(self, output_shape):
        ...
        self.input_shapes= [(output_shape[0] + self.params["order"], 0)]
...
def _make_windows(x, window):
    ...
    # edge padding (same behavior as before)
    pad = w // 2
...
class EMAPrimitive(UnaryPrimitive):
    def __init__(self, output_shape):
        ...
        self.name = "ema"
        self.params = {
            "span": np.random.randint(2, output_shape[0] // 2 + 2)
        }
        self.input_shapes= [output_shape]
...
class GaussianSmoothPrimitive(UnaryPrimitive):
    ...
    def __call__(self, x: list) -> np.ndarray:
        ...
        for j in range(m):
            ...
            output[:, j] = np.convolve(col_pad, kernel, mode="valid")
